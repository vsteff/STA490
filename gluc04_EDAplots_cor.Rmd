...
p1 <- xyplot(Glu ~ Time, groups = PID, 
       ...
       type = "b", pch = "*", cex = 5, col = rainbow(50),
       main = list("Glucose levels of 50 patients in 2014", cex = 4), 
       xlab = list("Month 2014", cex = 2),
       ylab = list("Glucose (mmol/l)", cex = 2),
       scales = list(cex = 2, y = list(log = TRUE, equispaced.log = FALSE)),
       ylim = range(glu$Glu), 
       panel = function(...) {
           panel.xyplot(...)
           ...
           panel.text(x = as.numeric(min(glu$Time)), 
                      ...
                      ranges,  # text
                      pos = 4, cex = 1.5)}  # left-bound
       )
print(p1)
# pdf("STA490/plots/p1.pdf", width = 20, height = 10); print(p1); dev.off()
```
